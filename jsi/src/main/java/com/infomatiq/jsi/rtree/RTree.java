...
public class RTree implements SpatialIndex, Serializable {
  ...
  public void write(DataOutput out){
	  try {
		...
		TIntObjectIterator<Node> i = nodeMap.iterator();
		while(i.hasNext()){
			i.advance();
			Node n = i.value();
			out.writeInt(i.key());
			out.writeInt(n.nodeId);
			out.writeInt(n.level);
			n.write(out);
		}
		
		out.writeInt(parents.size());
		for(int j = 0; j<parents.size();j++){
			out.writeInt(parents.pop());
		}
		
		out.writeInt(parentsEntry.size());
		for(int j = 0; j<parentsEntry.size();j++){
			out.writeInt(parentsEntry.pop());
		}
		
		out.writeInt(treeHeight);
		out.writeInt(rootNodeId);
		out.writeInt(size);
		out.writeInt(highestUsedNodeId);
  ...
  public void read(DataInput in){
	  try{
		  ...
		  for(int i = 0; i<readSize;i++){
			  int nodeKey = in.readInt();
			  int nodeId = in.readInt();
			  int nodeLevel = in.readInt();
			  Node n = new Node(nodeId,nodeLevel,maxNodeEntries);
			  n.read(in);
		  ...
		  for(int j = 0; j<readSize;j++){
			  parents.push(in.readInt());
		  }
		  
		  readSize = in.readInt();
		  for(int j = 0; j<readSize;j++){
			  parentsEntry.push(in.readInt());
		  }
		  
		  treeHeight = in.readInt();
		  rootNodeId = in.readInt();
		  size = in.readInt();
		  highestUsedNodeId = in.readInt();		  
