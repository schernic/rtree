...
public class Node implements Serializable {
  ...
  public int getLevel() {
	...
	public void write(DataOutput out) throws IOException {
		...
		out.writeFloat(mbrMinX);
		out.writeFloat(mbrMinY);
		out.writeFloat(mbrMaxX);
		out.writeFloat(mbrMaxY);

		out.writeInt(entriesMinX.length);
		for(int i = 0;i<entriesMinX.length;i++){
			out.writeFloat(entriesMinX[i]);
			out.writeFloat(entriesMinY[i]);
			out.writeFloat(entriesMaxX[i]);
			out.writeFloat(entriesMaxY[i]);
		}
	...
	public void read(DataInput in) throws IOException {
		...
		mbrMinX = in.readFloat();
		mbrMinY = in.readFloat();
		mbrMaxX = in.readFloat();
		mbrMaxY = in.readFloat();
		
		int arraysize = in.readInt();
		
		for(int i = 0;i<arraysize;i++){
			entriesMinX[i] = in.readFloat();
			entriesMinY[i] = in.readFloat();
			entriesMaxX[i] = in.readFloat();
			entriesMaxY[i] = in.readFloat();
		}
